...
func (r *UserRepository) FindByUsername(username string) (*models.User, error) {
	...
	err := r.db.Select("user_id, username, email, password_hash, display_name, avatar_url, last_active_at, created_at, updated_at, deleted_at").
...
func (r *UserRepository) FindByUsernameOrEmail(username, email string) (*models.User, error) {
	...
	query := `
		SELECT user_id, username, email, password_hash, display_name, avatar_url, last_active_at, created_at, updated_at, deleted_at 
		FROM users 
		WHERE username = ? 
		UNION 
		SELECT user_id, username, email, password_hash, display_name, avatar_url, last_active_at, created_at, updated_at, deleted_at 
...
func (r *UserRepository) FindPublicUserByID(userID string) (*models.PublicUser, error) {
	var user models.User
	err := r.db.Select("user_id, username, email, display_name, avatar_url, last_active_at, created_at, updated_at, deleted_at").
		Where("user_id = ?", userID).
		First(&user).Error
	if err != nil {
		return nil, err
